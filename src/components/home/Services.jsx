      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-extrabold text-[#0f172a] mb-6 italic">
            Web projects that create <br className="hidden md:block" />{" "}
            meaningful impact
          </h2>
          <p className="text-slate-500 text-lg max-w-2xl mx-auto font-medium">
            We plan, design, and refine websites that feel calm, confident, and
            commercially useful.
          </p>
        </motion.div>
          ...
          {serviceData.map((item) => (
            ...
            >
              ...
              <div className="relative z-10 flex flex-col h-full">
                <h3 className="text-3xl font-extrabold text-[#0f172a] mb-10">
                  {item.title}
                </h3>
                ...
                <ul className="space-y-5">
                  {item.items.map((li, i) => (
                    <li
                      key={i}
                      className="flex items-center gap-4 text-slate-700 font-bold tracking-tight"
                    >
                      <div
                        className={`flex-shrink-0 w-5 h-5 rounded-full ${item.checkBg} flex items-center justify-center`}
                      >
        ...
        >
          <Link
            to="/services"
            className="inline-flex items-center gap-2 bg-slate-900 text-white px-8 py-4 rounded-full font-bold hover:bg-brand-blue transition-all group shadow-lg"
          >
            View All Services
            <motion.span
              animate={{ x: [0, 5, 0] }}
