...
export const blogs = [
  {
    ...
    excerpt: "Discover the psychological triggers behind premium UI/UX and how they influence user decision-making.",
  ...
  },
  {
    ...
    excerpt: "Explore why modern startups are ditching static sites for high-performance, animated web experiences.",
    ...
    <div className="bg-[#F8FAFF] pt-32 pb-24 min-h-screen">
      <div className="max-w-7xl mx-auto px-6">
        <motion.div 
          ...
          className="mb-20"
        >
          <span className="text-blue-600 text-xs font-black uppercase tracking-[0.3em] mb-4 inline-block italic">Our Journal</span>
          <h1 className="text-6xl md:text-8xl font-black text-slate-900 tracking-tighter leading-none mb-8">
            Digital <span className="text-slate-300">Insights.</span>
          </h1>
        </motion.div>
          ...
          {blogs.map((post, i) => (
            ...
            >
              ...
              <div className="p-10">
                ...
                <h3 className="text-2xl font-black text-slate-900 mb-4 group-hover:text-blue-600 transition-colors">{post.title}</h3>
                <p className="text-slate-500 font-medium mb-8 line-clamp-2">{post.excerpt}</p>
                <Link to={`/blog/${post.id}`} className="inline-flex items-center gap-3 font-black text-xs uppercase tracking-widest group/btn">
                  Read More 
